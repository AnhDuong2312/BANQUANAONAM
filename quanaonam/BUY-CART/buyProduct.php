...
if (isset($data['user_id'], $data['product_id'], $data['product_name'], $data['product_image'], $data['size'], $data['quantity'], $data['price'])) {
    ...
    $query = "INSERT INTO order_detail (user_id, product_id, product_name, product_image, size, quantity, price)
              VALUES ('$userId', '$productId', '$productName', '$productImage', '$size', '$quantity', '$price')";

    if ($conn->query($query) === TRUE) {
        echo json_encode(['success' => true, 'message' => 'Sản phẩm đã được mua thành công.']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Lỗi khi mua sản phẩm ' . $conn->error]);
    }
} else {
